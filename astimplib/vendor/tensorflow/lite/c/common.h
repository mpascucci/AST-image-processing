...
#define TF_LITE_ENSURE(context, a)                                      \
    do {                                                                \
        if (!(a)) {                                                     \
            (context)->ReportError((context), "%s:%d %s was not true.", \
                                   __FILE__, __LINE__, #a);             \
            return kTfLiteError;                                        \
        }                                                               \
    } while (0)
...
#define TF_LITE_ENSURE_EQ(context, a, b)                                   \
    do {                                                                   \
        if ((a) != (b)) {                                                  \
            (context)->ReportError((context), "%s:%d %s != %s (%d != %d)", \
...
#define TF_LITE_ENSURE_TYPES_EQ(context, a, b)                              \
    do {                                                                    \
        if ((a) != (b)) {                                                   \
            (context)->ReportError(                                         \
                (context), "%s:%d %s != %s (%s != %s)", __FILE__, __LINE__, \
...
typedef struct {
    ...
    // Opaque data provided to the node if the node is a builtin. This is
    // usually a structure defined in builtin_op_data.h
    void* builtin_data;
...
typedef struct TfLiteContext {
    ...
    // `Free` method. Caller needs to make sure that that `buffer` is allocated
    // by `AllocateOpData` method. On TFL, it will free the buffer, and for
    // micro, this method is a no-op. WARNING: This is an experimental interface
    ...
    // change.
    TfLiteStatus (*AllocateTemporaryTensor)(
        struct TfLiteContext* ctx, TfLiteNode* node, int dims, int* shape,
        TfLiteType data_type, TfLiteAllocationType allocation_type,
    ...
    // Resize the memory pointer of the `tensor`. This method behaves the same
    // as `ResizeTensor`, except that it makes a copy of the shape array
    // internally so the shape array could be deallocated right afterwards.
    // WARNING: This is an experimental interface that is subject to change.
...
typedef struct TfLiteRegistration {
    ...
    // field, accessible within prepare and invoke functions below. NOTE: if the
    // data is already in the desired format, simply implement this function to
    // return `nullptr` and implement the free function to be a no-op.
    ...
    // in order to group executions together. Providing a value here will cause
    // a given op to appear multiple times is the profiling report. This is
...
typedef struct TfLiteDelegate {
    ...
    // 'tensor'. This cannot be null. The delegate is allowed to allocate the
    // raw bytes as long as it follows the rules for kTfLiteDynamic tensors.
